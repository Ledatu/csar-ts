...
describe("createAssertion", () => {
  it("creates a valid JWT for ED25519 key", async () => {
    ...
    const token = await createAssertion(key, "https://sts.example.com/sts/token");
    ...
    expect(payload.aud).toBe("https://sts.example.com/sts/token");
  ...
  it("generates unique jti for each assertion", async () => {
    const key = await generateTestKey("ED25519");
    const token1 = await createAssertion(key, "https://sts.example.com/sts/token");
    const token2 = await createAssertion(key, "https://sts.example.com/sts/token");
  ...
  it("throws ASSERTION_SIGN_FAILED for invalid private key", async () => {
    ...
    await expect(
      createAssertion(broken, "https://sts.example.com/sts/token"),
    ).rejects.toThrow(CsarAuthError);

    try {
      await createAssertion(broken, "https://sts.example.com/sts/token");
    } catch (err) {
